...
class QuizQuestionsActivity : AppCompatActivity() {
        ...
        btnSubmit?.setOnClickListener {
            if (!isAnswerChecked) {
                ...
                if (!anyAnswerIsChecked) {
                    Toast.makeText(this, "SVP, selectionnez une alternative", Toast.LENGTH_SHORT).show()
                } else {
                    ...
                    btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "FINISH" else "Au Question Suivant"
    ...
    private fun updateQuestion() {
        ...
        btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "FINISH" else "Envoyer"
    }
